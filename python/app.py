...
from gpu_backtester import MACrossStrategy, HeterogeneousBacktester
from metrics_visual import calculate_metrics
...
st.markdown("基于 **Pybind11 零拷贝** 与 **纯向量化撮合**，实现百万级数据亚毫秒级回测。")
...
if st.sidebar.button("▶️ 立即运行极速回测", use_container_width=True):
    ...
    with st.spinner('正在进行异构计算与向量化撮合...'):
        ...
        _, data_matrix = generate_massive_mock_data(num_stocks, num_days)
        
        # 2. 运行回测
        strategy = MACrossStrategy(window=ma_window)
        backtester = HeterogeneousBacktester(data_matrix, engine_type=engine_type, 
                                             commission=commission, slippage=slippage)
        
        # 获取耗时和每日收益率
        calc_time, port_returns = backtester.run(strategy)
